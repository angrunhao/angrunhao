...
def create_bootstrapping_treasury_zero_curve(server):
    ...
    dashApp = dash.Dash(server=server,
                        routes_pathname_prefix="/btzc/",
    ...
    animatedFig = go.Figure(data=[go.Scatter(x=spotData.iloc[0].index, y=spotData.iloc[0].values, line={"color": "rgb(67,67,67)"}, mode="lines", name="Spot"),
                                  go.Scatter(x=forwardData.iloc[0].index, y=forwardData.iloc[0].values, line={
                                             "color": "rgb(0,38,77)"}, mode="lines", name="T+1 Forward"),
    ...
    dashApp.layout = html.Div(children=[
        ...
        dcc.Slider(id="date-slider-input", min=0, max=len(dateList) - 1,
                   value=0, step=1)])
    #intialize callbacks
    init_callbacks(dashApp, spotData, forwardData,
                   ymax, ymin, colors, dateList)

    return dashApp.server


def init_callbacks(dashApp, spotData, forwardData, ymax, ymin, colors, dateList):
    @dashApp.callback([Output(component_id="date-output", component_property="children"),
